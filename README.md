...
截至 2026-05-30，以下 7 个 `rcore-os/tgoskits` PR 均已合并，另有 1 个 cgroup 后续 PR 已提交并处于 review 中：
...
| [#1045](https://github.com/rcore-os/tgoskits/pull/1045) | 支持 cgroup2 `cgroup.procs` 进程迁移和 `/proc/[pid]/cgroup` 视图 | Open |

GitHub PR 统计口径下，已合并的 7 个 PR 共涉及 50 个变更文件、17 个提交，合计 `+4007/-224` 行。新提交的 #1045 当前包含 5 个提交、7 个变更文件，统计为 `+392/-7` 行，因尚未合入，单独作为后续进展列示。上述统计包含测试代码、QEMU 配置和文档式 PR 描述中的相关实现背景，不等同于净新增功能代码行数。
...
公开 Issue [rcore-os/tgoskits#582](https://github.com/rcore-os/tgoskits/issues/582) 用于持续跟踪后续工作。比赛结束后，后续重点仍将围绕 StarryOS 的 cgroup 支持推进；

当前 #1045 已提交 `cgroup.procs` 迁移和 `/proc/[pid]/cgroup` 支持，后续仍将继续推进 fork/exit membership 生命周期、pids controller 等方向。
