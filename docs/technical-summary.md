...
本次成果主要由 7 个已合并的 `rcore-os/tgoskits` PR 组成，覆盖 StarryOS 的信号测试、futex / robust-list Linux ABI、nginx 运行兼容性、cgroup2 初始支持与层级目录能力。另有 1 个 cgroup 后续 PR 已提交并处于 review 中，用于继续推进 `cgroup.procs` 进程迁移和 `/proc/[pid]/cgroup` 支持；同时有 1 个公开 Issue 用于持续追踪后续 cgroup 工作。

这些贡献的共同特点是：先用用户态或组件级测试固化可观察行为，再做最小语义修复，并在 PR 中明确说明未覆盖范围。报告按已合并代码实际达到的能力边界进行总结。

截至 2026-05-30，`tgoskits` 侧 7 个 PR 已合并，GitHub 统计合计：
...
另有 open 状态的 #1045 当前包含 5 个提交、7 个变更文件，统计为 `+392/-7` 行。因该 PR 尚未合入，本报告将它作为“后续进展”单独说明，不计入已合并成果统计。

这些统计包含测试、QEMU 配置和必要的支撑代码。本报告重点解释每个 PR 实际解决了什么问题、怎么验证，以及哪些能力仍未实现。
...
| [#1045](https://github.com/rcore-os/tgoskits/pull/1045) | Open | cgroup2 `cgroup.procs` 迁移与 `/proc/[pid]/cgroup` | 7 files, +392/-7 | 四架构 `cgroup-basic`，62 pass；`clippy` 13/13 |
...
本质问题是测试把 x86_64 的特殊返回路径当作所有架构的通用行为。旧测试会让 aarch64 下 `restore()` 看到错误的 `SignalFrame` 起点，导致稳定失败。
...
该 PR 不覆盖完整 futex 支持。在保持修复合理的边界内，说明 PI futex、`FUTEX_WAKE_OP` 等复杂语义未覆盖。
...
PR 没有实现真实 `SIGIO` 投递，也没有把普通文件、目录 fd 的 `FIOASYNC` 纳入修复范围。在克制的合理范围内修复是因为 nginx 初始化需要的只是 fd flag / owner 状态可设置，而非完整异步信号通知机制。
...
边界：没有实现 UDP、RAW、AF_UNIX datagram 的 `FIONREAD`；没有实现 Linux AIO syscall family；没有重构网络栈读路径；nginx smoke 后续仍暴露 keep-alive 两连请求的独立问题。这些不是本 PR 的完成项。

### 4.6 cgroup2 初始 root 文件支持：PR #989

PR 链接：[#989](https://github.com/rcore-os/tgoskits/pull/989)

PR #989 是 StarryOS cgroup 支持的第一阶段提交。它的目标是落地可验证的最小 cgroup2 语义。
...
该 PR 有意不实现 cgroup 层级、controller 注册、进程迁移、`/proc/[pid]/cgroup`、pids/cpu/memory controller、cgroup namespace、`clone3.cgroup` 或 `CLONE_INTO_CGROUP`。这个边界用于避免“假的 Docker 支持”。
...
该 PR 仍未实现 per-process cgroup membership、`cgroup.procs` 迁移、fork/exit 继承与清理、`/proc/[pid]/cgroup`、controller 注册与接口文件。它是 hierarchy 基础，依然还不是完整 cgroup，用于给后续工作铺路。

### 4.8 cgroup2 process membership 与 procfs 视图：PR #1045

PR 链接：[#1045](https://github.com/rcore-os/tgoskits/pull/1045)

PR #1045 基于 #1015 继续推进 cgroup 支持，当前状态为 open。#1015 已经支持 cgroup2 mount、root interface files 和 child hierarchy，但用户态仍无法通过 `cgroup.procs` 把进程迁移到 child cgroup，也无法通过 `/proc/[pid]/cgroup` 观察进程所属 cgroup。#1045 的目标是补齐这一层 process membership 基础语义。

实现层面，该 PR 主要包含三部分。

第一，cgroup core：

- 新增 process membership API，支持 process 注册、注销和迁移。
- `cgroup.procs` read 改为按真实 live process membership 输出，每行一个 TGID。
- `cgroup.procs` write 支持写入 live TGID，把整个 process membership 迁移到目标 cgroup。
- 对空白、非数字、溢出和 `0` 输入返回 `EINVAL`；不存在 TGID 返回 `ESRCH`。
- populated child cgroup 的 `rmdir` 使用真实 live count 判定，返回 `EBUSY` 并保留目录。
- 新增 procfs cgroup 文本生成，格式为 `0::<path>\n`。

第二，task 生命周期接入：

- `ProcessData` 新增当前 cgroup id。
- 新建 process 默认进入 root cgroup，并注册 root live count。
- `ProcessData::Drop` best-effort 注销当前 cgroup live count，避免计数泄漏。

第三，procfs 接入：

- `/proc/[pid]` 和 `/proc/self` 目录新增只读 `cgroup` 文件。
- live process 读取 `/proc/[pid]/cgroup` 时返回当前 global cgroup hierarchy path。

`cgroup-basic` 本轮扩展到 62 项，新增覆盖 `/proc/self/cgroup` 初始值、非法 `cgroup.procs` 写入、当前 TGID 迁移到 child、root 与 child `cgroup.procs` 可见性变化、populated child `rmdir` 返回 `EBUSY`、迁回 root 后 child 可删除、`cgroup.controllers` 写入返回 `EACCES`，以及 cgroup v1 mount 继续返回 `ENODEV`。

验证记录包括 `cargo fmt`、`cargo xtask clippy --package starry-kernel`，以及 x86_64、aarch64、riscv64、loongarch64 四架构 `cgroup-basic` QEMU 测试。PR 描述记录的结果为 `starry-kernel clippy: 13/13 checks passed`，四架构均为 `62 pass, 0 fail`。

该 PR 仍保留明确边界：尚未实现 fork/clone 继承父进程 cgroup，尚未把 exit、zombie、reap 后 `cgroup.procs` 的用户态可见语义作为测试契约，尚未覆盖 `/proc/[other-pid]/cgroup` 的独立用户态正例，也尚未实现 `CLONE_NEWCGROUP`、`CLONE_INTO_CGROUP`、`clone3.cgroup`、cgroup namespace、threaded cgroup、`cgroup.threads` 或 pids/cpu/memory 等 controller。

## 5. 进展追踪 Issue：#582

Issue 链接：[#582](https://github.com/rcore-os/tgoskits/issues/582)

该 Issue 用于公开记录后续聚焦 cgroup 的工作进展。Issue 中已经记录了 #545、#657、#796、#869、#989、#1015 的阶段性完成情况，并补充了 cgroup 支持调研与实施路线；#1045 则是沿该路线继续提交的 `cgroup.procs` membership 与 procfs cgroup 视图工作。
...
cgroup 相关 PR 将 StarryOS 从没有 `cgroup2` mount 支持推进到：
...
- 已提交 `cgroup.procs` live membership 迁移和 `/proc/[pid]/cgroup` 视图支持。
- 四架构 QEMU 测试有独立 `cgroup-basic` case。

这仍然距离 Docker 所需能力很远，但它已经建立了后续 fork/exit membership 生命周期、procfs 完整语义和 pids controller 的基础结构和测试入口。
...
- `cgroup-basic`：cgroup2 mount/root/hierarchy、process membership 和 procfs cgroup 视图回归。

这些测试不仅证明当前修复有效，也能在后续迭代中防止回归。

## 8. 未完成边界

本节专门列出未完成内容，避免夸大成果造成实际工作中的误会。
...
- #1045 已提交 per-process cgroup membership、写 `cgroup.procs` 迁移进程和 `/proc/[pid]/cgroup` 支持，但尚未合入。
...
因此，本次已合并成果只能描述为“cgroup2 初始支持和 hierarchy 基础能力”；#1045 是后续 membership/procfs 能力的 open PR，仍不能描述为“已支持 Docker”。

## 9. 持续贡献方向

比赛结束后，本人将继续沿 Issue #582 的 slice 路线推进。当前 #1045 已提交 `cgroup.procs` 迁移和 `/proc/[pid]/cgroup` 支持，后续准备聚焦以下方向：

1. 推进 #1045 review、冲突处理和合入。
2. fork/clone 继承 membership，exit/zombie/reap 可见语义和清理。
3. pids controller：`pids.current`、`pids.max` 和 fork 超限失败。
4. 再评估 `clone3.cgroup`、cgroup namespace、systemd 兼容文件或 CPU accounting。

在后续推进中，本人会继续保持当前原则：新增每个 cgroup 文件都说明是真语义、只读兼容，还是未支持并返回错误；不使用“写入成功但无实际效果”的方式伪装兼容。

## 10. 结语

本次成果可以概括为：围绕 StarryOS 的 Linux 兼容性和 Docker 前置基础设施，完成了一组已合并的、带测试的上游 PR。

其中 futex / robust-list 工作修复了同步原语和 robust mutex 相关基础 ABI；nginx 工作推进了真实用户态程序的网络 socket ioctl 兼容；cgroup 工作从无到有建立了 cgroup2 mount/root/hierarchy 的最小可验证基础，并已继续提交 membership/procfs 方向的 open PR；Issue #582 则承接后续持续贡献。
